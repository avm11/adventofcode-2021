...
def is_hit_target(target_area, pos, vx, vy):
    ...
    while not in_target(target_area, pos) and not miss_target(target_area, pos):
        ...
        if pos.y > max_height:
            max_height = pos.y
    return (in_target(target_area, pos), max_height)


target_str = sys.stdin.readline().strip()
target_area = parse_target_area(target_str)
print(target_area)
max_height = max_vx = max_vy = 0
lt, rb = target_area
for vy in range(rb.y, rb.x + 1):
    for vx in range(0, rb.x + 1):
        is_hit, try_max_height = is_hit_target(target_area, Pt(0, 0), vx, vy)
        if is_hit and try_max_height > max_height:
            max_height = try_max_height
            max_vx, max_vy = vx, vy

print(max_height, max_vx, max_vy)
