...
current_paths = [(["start"], False)]
paths = []
while current_paths:
    new_current_paths = []
    for current_path, has_double in current_paths:
        last_node = current_path[-1]
        for next_node in cave_map[last_node]:
            if next_node == "end":
                paths.append(current_path + [next_node])
            elif is_big_node(next_node):
                new_current_paths.append((current_path + [next_node], has_double))
            elif next_node not in current_path:
                new_current_paths.append((current_path + [next_node], has_double))
            elif not has_double:
                new_current_paths.append((current_path + [next_node], True))
